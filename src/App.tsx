...
function App() {
  ...
  return <div className="site-shell">
    ...
    <main>
      ...
      <section id="office" className="office section section-charcoal"><div className="page-width"><div className="office-head"><div><p className="eyebrow gold-text">04 / Our office</p><h2>A professional space.<br /><em>A committed legal team.</em></h2></div><p>Our office imagery below is professional stock photography used as a placeholder until the firm’s own photographs are supplied.</p></div><div className="gallery"><div className="gallery-feature"><AnimatePresence mode="wait"><motion.img key={officeIndex} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .35 }} src={images.office[officeIndex].src} alt={images.office[officeIndex].alt} /></AnimatePresence><div className="gallery-controls"><button onClick={() => setOfficeIndex((officeIndex - 1 + images.office.length) % images.office.length)} aria-label="Previous office image"><ChevronLeft /></button><span>{String(officeIndex + 1).padStart(2, '0')} / {String(images.office.length).padStart(2, '0')}</span><button onClick={() => setOfficeIndex((officeIndex + 1) % images.office.length)} aria-label="Next office image"><ChevronRight /></button></div></div><div className="gallery-thumbs">{images.office.map((image, index) => <button className={index === officeIndex ? 'active' : ''} key={image.src} onClick={() => setOfficeIndex(index)}><img src={image.src} alt={image.alt} /></button>)}</div></div></div></section>
